...
def forward_regression(X, y, initial_list=[], threshold_in=0.1, verbose=True):
...
data = pd.read_csv("filterTotal_TPM_ART.csv", sep=',', header=0, low_memory=False)
data.pop("SID")
#data.pop("gene_id")
data.pop("Gene")
Y_train = data.pop("ART")
